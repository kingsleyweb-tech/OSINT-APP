...
export const PlatformIcon: React.FC<PlatformIconProps> = ({ 
  ...
  // 16. Domain / WHOIS / Infrastructure (Teal Network)
  if (p.includes('domain') || p.includes('whois') || p.includes('dns')) {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#00B4D8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
  ...
  // 17. Website Favicon or Clean Fallback
  let targetDomain = domain;
  if (!targetDomain && (p.includes('.com') || p.includes('.org') || p.includes('.io') || p.includes('.net'))) {
    targetDomain = p;
  }
